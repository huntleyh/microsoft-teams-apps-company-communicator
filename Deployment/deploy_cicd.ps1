...
function logout {
    ...
    WriteS -message "No logout as running in CI/CD"
}
...
# Initialize connections - Azure Az/CLI/Azure AD
    ...
    WriteI -message "Setting context to subscription $($parameters.subscriptionTenantId.value)"
    Set-AzContext -Subscription $parameters.subscriptionTenantId.value

    WriteI -message "Logging in with service principal"
    $user = az login --service-principal -u $parameters.azcliAppId.Value -p $parameters.azcliAppSecret.value --tenant $parameters.subscriptionTenantId.value
...
# Create or Update User App
    ...
    $app = GetAzureADApp $usersApp
    if($app -eq $null)
    {
        WriteE -message "User app does not exist in in Azure Active Directory. Exiting..."
        logout
        Exit
    }
    else {
        $userAppCred.appId = $env:$userAppId
        $userAppCred.password = $env:$userAppSecret
    }
	<#if($parameters.isUpgrade.Value){
